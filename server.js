...
<script>const submitUrl=${JSON.stringify(`/public/location-request/${encodeURIComponent(req.params.token)}/position`)};const button=document.getElementById('send'),statusEl=document.getElementById('status');button?.addEventListener('click',()=>{if(!navigator.geolocation){statusEl.className='status error';statusEl.textContent='Este teléfono no permite obtener ubicación.';return;}button.disabled=true;statusEl.className='status';statusEl.textContent='Obteniendo GPS de alta precisión…';navigator.geolocation.getCurrentPosition(async p=>{try{const r=await fetch(submitUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({latitude:p.coords.latitude,longitude:p.coords.longitude,accuracy:p.coords.accuracy})});const d=await r.json();if(!r.ok)throw new Error(d.message||'No fue posible enviar la ubicación');statusEl.className='status ok';statusEl.textContent='✓ Ubicación enviada correctamente. Ya puedes cerrar esta página.';}catch(e){button.disabled=false;statusEl.className='status error';statusEl.textContent=e?.message||'No fue posible enviar la ubicación. Intenta nuevamente.';}},e=>{button.disabled=false;statusEl.className='status error';statusEl.textContent=e.code===1?'Debes permitir el acceso a ubicación para continuar.':'No pudimos obtener un GPS preciso. Intenta nuevamente al aire libre.';},{enableHighAccuracy:true,timeout:20000,maximumAge:0});});</script></body></html>`);
  } catch (error) {
